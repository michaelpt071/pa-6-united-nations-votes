...
```{r}
unvotes |> 
  count(country, vote) |> # counting the amount of total votes based on country and vote type.
  group_by(country) |> #grouping the data by the country
  mutate(total = sum(n)) |> #find the total numbe of votes for each country 
  mutate(prop_vote = n/total) |> #adding a column based on the proprtion of the votes
  filter(country %in% c("United States", "Canada", 
                        "Germany", "France",
                        "Italy", "Japan",
                        "United Kingdom")) |> #Filters the countries that we want for the graph
  ggplot(aes(x = country, y = prop_vote, 
             fill = vote)) + #starting the plot
  geom_col(position = position_stack()) + #column geometry for the plot
  labs(x = "Group of Seven Countries",
       y = "Proportion of Votes",
       title = "Voting Record of the G7",
       fill = "Vote") + #labeling the data
  theme_minimal() +  #using minimal theme
  scale_fill_viridis_d(end = 0.8) + #give the end of rainbow color scheme
  coord_flip() #swaps to horizontal orientation and flips the x and y coordinates
```

> Describe what the graph above demonstrates above UN voting records for the G7 : Basically shows the proportion of the votes in the first world countries. Unites States is the country that has the lowest proportion for yes votes while also having the highest proportion of the no votes.
...
> Returns a list of all unique topics of UN votes in the dataset.
...
> Information regarding whether the vote was on an amendment or paragraph would provide some more useful information.
...
> No. There is no date information in the returned data.
...
> They do not match because it indicates that the UN votes more than once on an issue.
...
```{r}
roll_calls |> 
  left_join(issues, by = "rcid") #keeping all the data from roll calls and matching it by rcid.
```

```{r}
roll_calls |> 
  right_join(issues, by = "rcid") #keeping all the rows from issues and adding data based on rcid
```

```{r}
roll_calls |> 
  full_join(issues, by = "rcid") #keeping all the rows
```

```{r}
roll_calls |> 
  inner_join(issues, by = "rcid") #keeping all the rows that the data matches
```
...
> We should use the right joins because it is guarantteed to keeps all the data sets that we need.
...
roll_calls |> 
  ...
  mutate(issue = case_when(
    ...
    issue == "Palestinian conflict" ~ "Palestinian Conflict")) |>   # Shortening certian issue descriptions
  ...
  count(year, issue) |> # returns a count of votes on certian issues per year
  ggplot(aes(x = year, y = n, group = issue)) + #begins ggplot, defines universal aesthetics
  geom_line(aes(color = issue)) + #adds line geometry, sets aesthetic specific to the line plot
  labs(x = "Year", y = "Number of Votes",
...
> Arms control and nuclear weapon votes drop off near to the end of the Cold War in 1991. Additionally, votes about the Palestinian conflict rise along with tensions and escalating conflits in the middle east.
...
> The US is voting 'yes' less and less in recent years.
